...
class MessengerRecordDocumentEquality implements Equality<MessengerRecord> {
  const MessengerRecordDocumentEquality();

  @override
  bool equals(MessengerRecord? e1, MessengerRecord? e2) {
    const listEquality = ListEquality();
    return e1?.otheruserref == e2?.otheruserref &&
        e1?.myuserref == e2?.myuserref &&
        listEquality.equals(e1?.othermessage, e2?.othermessage) &&
        listEquality.equals(e1?.mymessage, e2?.mymessage);
  }

  @override
  int hash(MessengerRecord? e) => const ListEquality()
      .hash([e?.otheruserref, e?.myuserref, e?.othermessage, e?.mymessage]);

  @override
  bool isValidKey(Object? o) => o is MessengerRecord;
}
