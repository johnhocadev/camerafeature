...
class LodgeListRecordDocumentEquality implements Equality<LodgeListRecord> {
  const LodgeListRecordDocumentEquality();

  @override
  bool equals(LodgeListRecord? e1, LodgeListRecord? e2) {
    const listEquality = ListEquality();
    return e1?.userref == e2?.userref &&
        listEquality.equals(e1?.lodgeref, e2?.lodgeref) &&
        e1?.listtitle == e2?.listtitle;
  }

  @override
  int hash(LodgeListRecord? e) =>
      const ListEquality().hash([e?.userref, e?.lodgeref, e?.listtitle]);

  @override
  bool isValidKey(Object? o) => o is LodgeListRecord;
}
