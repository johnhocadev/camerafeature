...
class DisplayRecordDocumentEquality implements Equality<DisplayRecord> {
  const DisplayRecordDocumentEquality();

  @override
  bool equals(DisplayRecord? e1, DisplayRecord? e2) {
    return e1?.post1 == e2?.post1 &&
        e1?.post2 == e2?.post2 &&
        e1?.recommend == e2?.recommend &&
        e1?.comment == e2?.comment;
  }

  @override
  int hash(DisplayRecord? e) =>
      const ListEquality().hash([e?.post1, e?.post2, e?.recommend, e?.comment]);

  @override
  bool isValidKey(Object? o) => o is DisplayRecord;
}
